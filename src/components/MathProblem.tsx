...
const MathProblem: React.FC<MathProblemProps> = ({ problem, show }) => {
  return (
    <div className={`absolute top-10 left-0 right-0 z-30 transition-all duration-300 ${show ? 'opacity-100 transform-none' : 'opacity-0 -translate-y-10'}`}>
      <div className="pixel-border bg-white bg-opacity-90 p-3 mx-auto max-w-xs shadow-lg border-4 border-black animate-bounce-slow">
        <h3 className="text-4xl font-bold text-indigo-800 mb-2 pixel-text">{problem.question}</h3>
      </div>
      <div className="mt-8 flex justify-center space-x-2 relative">
        {problem.options.map((option, index) => (
          <div
            key={index}
            className="answer-option pixel-border text-2xl font-bold text-white bg-gradient-to-br from-[#F97316] to-orange-600 w-12 h-12 flex items-center justify-center"
            style={{ 
              right: '-100px',
